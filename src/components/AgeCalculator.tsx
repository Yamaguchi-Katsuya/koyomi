...
import { calculateAge, convertWarekiToSeireki, getEto } from '../utils/dateUtils';
import { validateWarekiYear, validateMonth, validateDay } from '../utils/validation';
...
const AgeCalculator: React.FC = () => {
    ...
    useEffect(() => {
        if (birthYear !== '') {
            ...
            const birthDate = new Date(year, (birthMonth as number) - 1, birthDay as number);
    ...
    useEffect(() => {
        if (warekiYear !== '') {
            ...
            if (!validation.valid) {
                ...
                setErrorMessage('');
                const seirekiYear = convertWarekiToSeireki(selectedEra, warekiYear as number);
                if (seirekiYear === '') {
                    ...
                    const day = validateDay(seirekiYear as number, month, birthDay as number);
                    setBirthMonth(month);
                    setBirthDay(day);
                    const birthDate = new Date(seirekiYear as number, (month === '' ? 1 : month) - 1, day === '' ? 1 : day);
                    setAge(calculateAge(birthDate));
    ...
    const handleMonthChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (birthYear !== '') {
            ...
            setBirthDay(day);
            const birthDate = new Date(birthYear as number, (month === '' ? 1 : month) - 1, day === '' ? 1 : day);
            setAge(calculateAge(birthDate));
        }
    };

    const handleDayChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const day = validateDay(birthYear as number, birthMonth, parseInt(e.target.value, 10));
        setBirthDay(day);
        if (birthYear !== '') {
